...
class TimerViewController: UIViewController {
    ...
    func getUserDefaults() {
        pomodoroTime = defaults.integer(forKey: "workTime") * 60
        shortBreakTime = defaults.integer(forKey: "shortBreak") * 60
        longBreakTime = defaults.integer(forKey: "longBreak") * 60
    ...
    func setupTimer() {
        ...
        timerType = .work
        currentModeLabel.text = "Focus Time"
    ...
    func pauseTimer() {
        timer!.invalidate()
        pauseStartButton.setTitle("Continue", for: .normal)
    ...
    func timerCompletedHandler() {
        ...
        if timerType == .work {
            ...
            if pomodoroCounter == pomodorosUntilLongBreak {
                pomodoroCounter = 0
                timerType = .long
                currentModeLabel.text = "Long Break"
                totalTime = longBreakTime
                circularProgressBarView.changeColor(longBreakColor)
            }
            else {
                timerType = .short
                currentModeLabel.text = "Short Break"
